...
GitHub Actions 配置为每 15 分钟触发一次；实际执行由 GitHub 调度，可能显著延迟，不能保证固定间隔。手机/电视仍需刷新订阅才能读到新列表。每次读取官方全赛季赛程，检查未来 7 天及仍在进行的比赛，再获取单场官方画质列表。每条源都检查 M3U8 和少量视频分片，验证成功才发布；默认每场只选最高可用画质。比赛结束会移出播放列表。为兼容 APTV 的订阅识别，播放列表至少保留两项：没有可播放直播时，显示“暂无可用直播／下场赛程”和“订阅使用提示”；只有一场可播比赛时，显示真实直播和“订阅使用提示”；两场及以上时仅列出真实直播。提示项播放本站生成的 12 秒说明视频，不是比赛直播。下场赛程从全赛季列表选择，不受未来 7 天检查窗口限制。
