...
def render_index(matches, meta):
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex, nofollow">
...
<script>async function copyUrl(file){{const url=new URL(file,location.href).href;document.getElementById('copied').textContent=url;try{{await navigator.clipboard.writeText(url);document.getElementById('copied').textContent='已复制：'+url}}catch(e){{document.getElementById('copied').textContent=url}}}}</script></body></html>'''
